...
function BeneficiaryHistory() {
  ...
  const getApplicantsInAYear= (year)=>{
    ...
    const Url = "http://localhost:5000/application/getPreBen?year="+ year
    axios.get(Url)
    .then(response => {  
...
<FormControl>
    ...
    total previous Beneficiaries
    {data.map((num) => (
      <h3 style={{color:'red'}}>{num.totalBeneficiaries}</h3>
    ))}
   <Box>
        <PageHeader pageTitle="previous Beneficiaries"/>
